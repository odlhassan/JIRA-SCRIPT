...
def _load_managed_project_display_names(db_path: Path) -> dict[str, str]:
    ...
    for row in rows:
        ...
        if not key:
            continue
        out[key] = _to_text(row["display_name"]) or _to_text(row["project_name"]) or key
    return out
...
def _build_payload(paths: dict[str, Path]) -> dict[str, Any]:
    ...
    managed_project_display_names = _load_managed_project_display_names(paths["capacity_db_path"])
    epic_release_map = _load_epic_release_map(paths["capacity_db_path"])
    for epic in epics:
        rel = epic_release_map.get(_to_text(epic.get("issue_key")).upper(), {})
        epic["release_number"] = _to_text(rel.get("release_number"))
        epic["release_date"] = _to_text(rel.get("release_date"))
        epic["actual_release_date"] = _to_text(rel.get("actual_release_date"))
        epic["release_state"] = _to_text(rel.get("release_state"))
    return {
...
<section class="panel story-page" data-story-page="1"><h2 class="page-title">1. Total work by projects</h2><div style="margin:0 0 8px;color:#334155;font-size:.78rem;font-weight:700;"><label style="display:inline-flex;align-items:center;gap:6px;cursor:pointer;"><input id="toggle-total-leaves-planned" type="checkbox" checked> Show Total Leaves Planned scorecard</label></div><div class="leadership-cards"><article class="kpi metric-capacity"><div class="label">Capacity</div><div class="value" id="kpi-total-capacity">0h</div><div class="metric-bar-wrap"><div class="metric-bar capacity" id="card-bar-capacity"></div></div></article><article class="kpi metric-planned" id="kpi-card-total-leaves-planned"><div class="label">Total Leaves Planned</div><div class="value" id="kpi-total-leaves-planned">0h</div><div class="metric-bar-wrap"><div class="metric-bar planned" id="card-bar-total-leaves-planned"></div></div></article><article class="kpi metric-availability"><div class="label">Availability</div><div class="value" id="kpi-capacity-after-leaves">0h</div><div class="metric-bar-wrap"><div class="metric-bar availability" id="card-bar-availability"></div></div></article><article class="kpi metric-planned"><div class="label">Planned Hours</div><div class="value" id="kpi-work-on-plate">0h</div><div class="metric-bar-wrap"><div class="metric-bar planned" id="card-bar-planned"></div></div></article><article class="kpi metric-actual"><div class="label">Actual Hours</div><div class="value" id="kpi-total-actual-project-hours">0h</div><div class="metric-bar-wrap"><div class="metric-bar actual" id="card-bar-actual"></div></div></article><article class="kpi metric-required"><div class="label">Plan Actual Difference</div><div class="value" id="kpi-hours-required-projects">0h</div><div class="metric-bar-wrap"><div class="metric-bar required" id="card-bar-required"></div></div></article></div><article class="story-card"><h3 class="story-title">Project Bullet Chart (Actual bar vs Planned red benchmark)</h3><div class="project-compare-chart" id="project-plan-vs-actual-chart"><div class="vlabel">No project data available.</div></div></article></section>
...
const scopedSubtasksCache=new Map();
const managedProjectDisplayNames=(payload&&typeof payload.managed_project_display_names==="object"&&payload.managed_project_display_names)||{};
function projectDisplayName(projectKey){const key=txt(projectKey).toUpperCase();return txt(managedProjectDisplayNames[key]||key)||key}
function epicMetadataMap(){const out=new Map();for(const epic of (Array.isArray(payload.epics)?payload.epics:[])){const key=txt(epic&&epic.issue_key).toUpperCase();if(!key)continue;out.set(key,epic)}return out}
function scopedSubtasksCacheKey(fromIso,toIso,mode){return [txt(fromIso),txt(toIso),txt(mode)||ACTUAL_MODE_DEFAULT].join("|")}
async function loadScopedSubtasks(fromIso,toIso,mode){if(!String(window.location.protocol||"").startsWith("http"))return null;const cacheKey=scopedSubtasksCacheKey(fromIso,toIso,mode);if(scopedSubtasksCache.has(cacheKey))return scopedSubtasksCache.get(cacheKey);const params=new URLSearchParams();params.set("from",txt(fromIso));params.set("to",txt(toIso));params.set("mode",txt(mode)==="planned_dates"?"extended":"log_date");const response=await fetch(`${SCOPED_SUBTASKS_ENDPOINT}?${params.toString()}`,{method:"GET",cache:"no-store"});const data=await response.json().catch(()=>({}));if(!response.ok||!data||data.ok===false)throw new Error(String(data&&data.error||"Failed to fetch scoped subtasks."));scopedSubtasksCache.set(cacheKey,data);return data}
function buildEpicRowsFromScopedPayload(scopedPayload){const rows=Array.isArray(scopedPayload&&scopedPayload.rows)?scopedPayload.rows:[];if(!rows.length)return[];const epicMeta=epicMetadataMap();const grouped=new Map();for(const row of rows){const epicKey=txt(row&&row.epic_key).toUpperCase();if(!epicKey)continue;const projectKey=txt(row&&row.project_key).toUpperCase();if(!projectKey||projectKey==="RLT")continue;const meta=epicMeta.get(epicKey)||{};let rec=grouped.get(epicKey);if(!rec){rec={issue_key:epicKey,project_key:projectKey||txt(meta.project_key).toUpperCase(),summary:txt(meta.summary),status:txt(meta.status),start_date:txt(meta.start_date),end_date:txt(meta.end_date),planned_hours:0,actual_hours:0,subtask_count:0,release_number:txt(meta.release_number),release_date:txt(meta.release_date),actual_release_date:txt(meta.actual_release_date),release_state:txt(meta.release_state)};grouped.set(epicKey,rec)}rec.planned_hours=+(num(rec.planned_hours)+num(row&&row.original_estimate_hours)).toFixed(2);rec.actual_hours=+(num(rec.actual_hours)+num(row&&row.logged_hours)).toFixed(2);rec.subtask_count+=1}return Array.from(grouped.values())}
...
function monthlyPlannedByProject(scopedPayload){const now=new Date();const currentStart=monthStartLocal(now);const previousStart=addMonthsLocal(currentStart,-1);const nextStart=addMonthsLocal(currentStart,1);const byProject={};const scopedEpicRows=buildEpicRowsFromScopedPayload(scopedPayload);const sourceRows=scopedEpicRows.length?scopedEpicRows:(Array.isArray(payload.planned_epic_rows)?payload.planned_epic_rows:[]);for(const row of sourceRows){const key=txt(row.project_key).toUpperCase();if(!key||key==="RLT")continue;const anchor=parseIso(row.end_date)||parseIso(row.start_date)||parseIso(row.planned_end)||parseIso(row.planned_start);if(!anchor)continue;const planned=Math.max(num(row.planned_hours),0);if(planned<=0)continue;if(!Object.prototype.hasOwnProperty.call(byProject,key))byProject[key]={project_key:key,previous:0,current:0,upcoming:0,total:0};if(anchor>=previousStart&&anchor<currentStart)byProject[key].previous+=planned;else if(anchor>=currentStart&&anchor<nextStart)byProject[key].current+=planned;else if(anchor>=nextStart)byProject[key].upcoming+=planned}const rows=[];let previousTotal=0,currentTotal=0,upcomingTotal=0;for(const key of Object.keys(byProject)){const rec=byProject[key];rec.previous=+num(rec.previous).toFixed(2);rec.current=+num(rec.current).toFixed(2);rec.upcoming=+num(rec.upcoming).toFixed(2);rec.total=+(rec.previous+rec.current+rec.upcoming).toFixed(2);if(rec.total<=0)continue;rows.push(rec);previousTotal+=rec.previous;currentTotal+=rec.current;upcomingTotal+=rec.upcoming}rows.sort((a,b)=>num(b.total)-num(a.total));const totalDefault=+(previousTotal+currentTotal+upcomingTotal).toFixed(2);const previousDefault=+num(previousTotal).toFixed(2);const currentDefault=+num(currentTotal).toFixed(2);const upcomingDefault=+num(upcomingTotal).toFixed(2);const baseContext={capacity:0,actual_hours:0,planned_leaves:0};const totalEval=evaluateManagedField("total_planned_hours",totalDefault,{...baseContext,planned_hours:totalDefault});const prevEval=evaluateManagedField("previous_month_planned_hours",previousDefault,{...baseContext,planned_hours:previousDefault});const currentEval=evaluateManagedField("current_month_planned_hours",currentDefault,{...baseContext,planned_hours:currentDefault});const upcomingEval=evaluateManagedField("upcoming_planned_hours",upcomingDefault,{...baseContext,planned_hours:upcomingDefault});return{rows,meta:{previous_label:monthLabel(previousStart),current_label:monthLabel(currentStart),upcoming_label:`${monthLabel(nextStart)} onward`,total:+num(totalEval.value).toFixed(2),previous:+num(prevEval.value).toFixed(2),current:+num(currentEval.value).toFixed(2),upcoming:+num(upcomingEval.value).toFixed(2)}}}
function epicMetrics(from,to,scopedPayload){const scopedEpicRows=buildEpicRowsFromScopedPayload(scopedPayload);if(scopedEpicRows.length){const out=[];let pending=0,closed=0,open=0,totalEstimate=0,totalLogged=0;for(const epic of scopedEpicRows){const logged=num(epic.actual_hours);const est=num(epic.planned_hours);const p=Math.max(est-logged,0);const s=statusClass(epic.status);if(s==="closed")closed+=1;if(s==="open")open+=1;pending+=p;totalEstimate+=est;totalLogged+=logged;out.push({...epic,epic_logged_hours:+logged.toFixed(2),pending_hours:+p.toFixed(2),status_bucket:s})}const required=totalEstimate-totalLogged;return{filtered:out,pending_required_hours:+pending.toFixed(2),hours_required_to_complete_epics:+required.toFixed(2),total_estimate_hours:+totalEstimate.toFixed(2),total_logged_hours:+totalLogged.toFixed(2),closed_resolved_count:closed,open_in_progress_count:open}}const by=payload.epic_logged_hours_by_key||{};const out=[];let pending=0,closed=0,open=0,totalEstimate=0,totalLogged=0;for(const epic of (Array.isArray(payload.epics)?payload.epics:[])){if(txt(epic.project_key).toUpperCase()==="RLT")continue;if(!(inRange(epic.start_date,from,to)||inRange(epic.end_date,from,to)))continue;const key=txt(epic.issue_key).toUpperCase();const logged=num(by[key]);const est=num(epic.original_estimate_hours);const p=Math.max(est-logged,0);const s=statusClass(epic.status);if(s==="closed")closed+=1;if(s==="open")open+=1;pending+=p;totalEstimate+=est;totalLogged+=logged;out.push({...epic,epic_logged_hours:+logged.toFixed(2),pending_hours:+p.toFixed(2),status_bucket:s})}const required=totalEstimate-totalLogged;return{filtered:out,pending_required_hours:+pending.toFixed(2),hours_required_to_complete_epics:+required.toFixed(2),total_estimate_hours:+totalEstimate.toFixed(2),total_logged_hours:+totalLogged.toFixed(2),closed_resolved_count:closed,open_in_progress_count:open}}
...
function renderLeadershipCards(availability,capacity,planned,totalLeavesPlanned,actual,required){const maxH=Math.max(num(availability),num(capacity),num(planned),num(totalLeavesPlanned),num(actual),Math.max(num(required),0),1);width("card-bar-availability",Math.max(num(availability),0),maxH);width("card-bar-capacity",Math.max(num(capacity),0),maxH);width("card-bar-planned",Math.max(num(planned),0),maxH);width("card-bar-total-leaves-planned",Math.max(num(totalLeavesPlanned),0),maxH);width("card-bar-actual",Math.max(num(actual),0),maxH);width("card-bar-required",Math.max(num(required),0),maxH);}
function renderProjectPlanVsActual(rows){const host=$("project-plan-vs-actual-chart");if(!host)return;const source=Array.isArray(rows)?rows:[];if(!source.length){host.innerHTML='<div class="vlabel">No project data available for the selected range.</div>';return}const top=source.slice(0,12);const max=Math.max(...top.map((r)=>Math.max(num(r.planned_hours),num(r.actual_hours))),1);const head=`<div class="bc-row"><div class="bc-head">RMIs</div><div class="bc-head">Due Date</div><div class="bc-head">Bullet Chart</div><div class="bc-head" style="text-align:right">Data Labels</div></div>`;const body=top.map((r)=>{const actual=num(r.actual_hours);const planned=num(r.planned_hours);const actualPct=((actual/max)*100).toFixed(2);const plannedPct=((planned/max)*100).toFixed(2);const due=txt(r.project_due_date)||"-";const plannedLabelPct=plannedPct==="0.00"?"0.80":plannedPct;return `<div class="bc-row"><div class="bc-label">${projectDisplayName(r.project_key)}</div><div class="bc-due">${due}</div><div class="bc-track"><div class="bc-actual" style="width:${actualPct}%"><div class="bc-bar-label">${hTxt(actual)}h</div></div><div class="bc-benchmark" style="left:${plannedPct}%"></div><div class="bc-benchmark-label" style="left:${plannedLabelPct}%">${hTxt(planned)}h</div></div><div class="bc-values">Actual ${hTxt(actual)}h | Planned ${hTxt(planned)}h</div></div>`}).join("");host.innerHTML=head+body}
...
function renderProjectPlannedScorecards(rows){const host=$("project-planned-scorecards");if(!host)return;const source=Array.isArray(rows)?rows:[];if(!source.length){selectedEpicProjectKey="";host.innerHTML='<div class="project-scorecards-empty">No project scorecards available for the selected range.</div>';return}if(selectedEpicProjectKey&&!source.some((r)=>txt(r.project_key).toUpperCase()===selectedEpicProjectKey))selectedEpicProjectKey="";const allActive=!selectedEpicProjectKey;const allPlanned=source.reduce((sum,r)=>sum+num(r.planned_hours),0);const allCount=source.reduce((sum,r)=>sum+num(r.epic_count),0);const allCard=`<button type="button" class="project-scorecard ${allActive?"active":""}" data-project-key=""><span class="k">All Projects</span><span class="v">${hTxt(allPlanned)}h</span><span class="n">${Math.round(allCount)} epics</span></button>`;const cards=source.map((r)=>{const key=txt(r.project_key).toUpperCase();const active=selectedEpicProjectKey===key?"active":"";return `<button type="button" class="project-scorecard ${active}" data-project-key="${key}" style="border-left:5px solid ${projectColor(key)}"><span class="k">${projectDisplayName(key)}</span><span class="v">${hTxt(r.planned_hours)}h</span><span class="n">${Math.round(num(r.epic_count))} epics</span></button>`}).join("");host.innerHTML=allCard+cards;for(const btn of Array.from(host.querySelectorAll(".project-scorecard"))){btn.addEventListener("click",()=>{const key=txt(btn.getAttribute("data-project-key")).toUpperCase();selectedEpicProjectKey=key||"";renderProjectPlannedScorecards(projectScorecardRows(latestEpicRows));renderEpicPlannedHoursBars(latestEpicRows,selectedEpicProjectKey)})}}
const RELEASE_STATE_COLORS={Released:"#4caf50",Planned:"#2196f3",Rescheduled:"#ff9800",Shelfed:"#9e9e9e"};function releaseStateBadge(state){const s=txt(state);if(!s)return"";const bg=RELEASE_STATE_COLORS[s]||"#9e9e9e";return `<span style="display:inline-block;padding:1px 6px;border-radius:999px;background:${bg};color:#fff;font-size:.64rem;font-weight:700;vertical-align:middle;margin-left:4px">${s}</span>`}function epicReleaseBadge(r){const rn=txt(r.release_number);if(!rn)return"";const rd=txt(r.release_date)||"";const ard=txt(r.actual_release_date)||"";const state=txt(r.release_state)||"";const datePart=ard?`Actual: ${ard}`:(rd?`Planned: ${rd}`:"");const title=`Release ${rn}${datePart?` | ${datePart}`:""}${state?` | ${state}`:""}`;return `<span style="display:inline-flex;align-items:center;gap:3px;margin-left:6px;vertical-align:middle;font-size:.67rem;color:#475569;font-weight:700" title="${title}"><span style="opacity:.6">📦</span>${rn}${releaseStateBadge(state)}</span>`}function renderEpicPlannedHoursBars(epicRows,projectFilter){const host=$("epic-planned-hours-chart");const labelHost=$("epic-project-filter-label");if(!host)return;const filter=txt(projectFilter).toUpperCase();const source=Array.isArray(epicRows)?epicRows:[];const filtered=filter?source.filter((row)=>txt(row.project_key).toUpperCase()===filter):source;if(labelHost)labelHost.textContent=filter?`Showing project: ${projectDisplayName(filter)}`:"Showing all projects";if(!filtered.length){host.innerHTML='<div class="epic-empty">No epics matched the selected range.</div>';return}const max=Math.max(...filtered.map((r)=>num(r.planned_hours)),1);host.innerHTML=filtered.map((r)=>{const planned=num(r.planned_hours);const pctValue=(planned/max)*100;const pct=pctValue.toFixed(2);const labelLeft=Math.min(Math.max(pctValue+0.8,1.2),96);const projectKey=txt(r.project_key).toUpperCase();const color=projectColor(projectKey);const label=`${txt(r.issue_key)} - ${txt(r.summary)||"-"}`;return `<div class="epic-row"><div class="epic-label"><span class="epic-project-tag">${projectDisplayName(projectKey)}</span>${label}${epicReleaseBadge(r)}</div><div class="epic-track"><div class="epic-bar" style="width:${pct}%;background:${color}"></div><div class="epic-value-pin" style="left:${labelLeft}%">${hTxt(planned)}h</div></div></div>`}).join("")}
...
function setTotalLeavesPlannedVisibility(show){const card=$("kpi-card-total-leaves-planned");if(card)card.style.display=show?"":"none"}
function initTotalLeavesPlannedToggle(){const toggle=$("toggle-total-leaves-planned");if(!toggle)return;const stored=localStorage.getItem(TOGGLE_TOTAL_LEAVES_PLANNED_KEY);const show=stored===null?true:stored!=="0";toggle.checked=show;setTotalLeavesPlannedVisibility(show);toggle.addEventListener("change",()=>{const next=!!toggle.checked;setTotalLeavesPlannedVisibility(next);localStorage.setItem(TOGGLE_TOTAL_LEAVES_PLANNED_KEY,next?"1":"0")})}
...
const scopedPayload=await loadScopedSubtasks(fromDate,toDate,actualMode).catch((err)=>{setProfileStatus(`Scoped-subtask fallback: ${txt(err?.message||err)}`);return null;});
const available=num(cap?.metrics?.available_capacity_hours);
const leave=cap?.leave_metrics||{};
const plannedLeaves=num(leave.planned_taken_hours)+num(leave.planned_not_taken_hours||leave.not_yet_taken_hours);
const afterDefault=+(available-plannedLeaves).toFixed(2);
const work=plannedWorkOnPlate(from,to,scopedPayload);
const actualProjects=totalActualProjectHours(from,to,actualAgg,scopedPayload);
const hoursRequiredDefault=+(work-actualProjects).toFixed(2);
const totalLeavesPlanned=plannedLeaves;
const context={capacity:available,planned_hours:work,actual_hours:actualProjects,planned_leaves:plannedLeaves};
...
const compareRows=projectCompareRows(from,to,actualAgg,scopedPayload);
const monthlyPlanned=monthlyPlannedByProject(scopedPayload);
$("kpi-capacity-after-leaves").textContent=`${hTxt(after)}h`;
$("kpi-total-capacity").textContent=`${hTxt(available)}h`;
$("kpi-work-on-plate").textContent=`${hTxt(work)}h`;
$("kpi-total-leaves-planned").textContent=`${hTxt(totalLeavesPlanned)}h`;
...
renderLeadershipCards(after,available,work,totalLeavesPlanned,actualProjects,hoursRequiredProjects);
...
latestEpicRows=epicPlannedRowsByProject(from,to,scopedPayload);
