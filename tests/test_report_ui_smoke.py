...
class ReportUiSmokeTests(unittest.TestCase):
    ...
    def test_nested_capacity_endpoints_present(self):
        ...
        self.assertIn("const leaveSubtaskRows = Array.isArray(reportData.leave_subtask_rows)", html)
        self.assertIn("if (loggedHours > 0)", html)
        self.assertIn("startDate && startDate.getTime() > today.getTime() && estimateHours > 0", html)
    ...
    def test_manage_fields_page_and_settings_links_exist(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as td:
            ...
            self.assertIn('href="/settings/manage-fields"', capacity_html)
            perf_html = client.get("/settings/performance").get_data(as_text=True)
            self.assertIn('href="/settings/manage-fields"', perf_html)
            entities_html = client.get("/settings/report-entities").get_data(as_text=True)
            self.assertIn('href="/settings/manage-fields"', entities_html)
            self.assertIn('href="/settings/projects"', entities_html)
            self.assertIn('href="/settings/epics-dropdown-options"', entities_html)
            self.assertIn('href="/settings/epic-phases"', entities_html)
            self.assertIn('href="/settings/epics-management"', entities_html)
            ...
            self.assertIn("Project / Product Categorization / Epic", epics_html)
            ...
            self.assertIn("Manage Epic Phases", epics_html)
