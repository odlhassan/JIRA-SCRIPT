...
class ReportUiSmokeTests(unittest.TestCase):
    ...
    def test_canonical_refresh_settings_formats_timestamps_for_display(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as td:
            ...
            self.assertNotIn('id="create-db-backup"', html)
            self.assertIn("create_db_backup: false", html)
            self.assertIn('id="fetch-smart-btn"', html)
            self.assertIn('id="compute-latest-btn"', html)
            self.assertIn('id="finish-refresh-btn"', html)
            self.assertIn('class="control-group scope-group"', html)
            self.assertIn('id="scope-group-title">Refresh scope', html)
            self.assertIn('class="control-group primary-actions"', html)
            self.assertIn('id="complete-refresh-title">Complete refresh', html)
            self.assertIn("Advanced workflow: run Fetch and report build separately", html)
            self.assertIn('class="advanced-flow"', html)
            self.assertIn('id="fetch-only-title">1. Fetch Jira data only', html)
            self.assertIn('id="build-reports-title">2. Build and publish reports', html)
            self.assertIn('id="run-controls-title" class="utility-label">Run controls', html)
            self.assertIn('id="export-tools-title" class="utility-label">Export tools', html)
            self.assertIn("Jira fetch is complete; reports are not updated yet.", html)
            self.assertIn("Waiting: Build reports", html)
            ...
            self.assertIn('const COMPUTE_CANCEL_API = "/api/canonical-compute/cancel";', html)
            self.assertIn('id="cancel-btn-label"', html)
            self.assertIn('computeBusy ? "Cancel Report Build" : "Cancel Refresh"', html)
            self.assertIn('id="meta-reconciliation"', html)
            self.assertIn('id="meta-db-backup"', html)
            self.assertIn("<strong>Report Build</strong>", html)
            self.assertIn("formatStageCountValue", html)
            ...
            self.assertIn("Reports still show the previous published version", html)
