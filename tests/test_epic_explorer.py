...
class EpicExplorerTests(unittest.TestCase):
    ...
    def test_html_contains_drilldown_drawer_charts_and_csv_controls(self):
        ...
        self.assertIn('class="planned-dates-col"', html)
        self.assertIn("formatDateMmmDdYyyy", html)
        self.assertIn('["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]', html)
        self.assertIn('class="planned-date-label">S:</span>', html)
        self.assertIn('class="planned-date-label">D:</span>', html)
        self.assertIn("`${month}-${parts.day}-${parts.year}`", html)
        self.assertNotIn("formatDateMonthDdYyyy", html)
        self.assertNotIn("`${dateText(row.planned_start)} to ${dateText(row.planned_due)}`", html)
