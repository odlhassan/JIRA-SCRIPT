...
class EpicExplorerTests(unittest.TestCase):
    def test_payload_rolls_up_full_epic_data_and_filters_only_epic_scope(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as td:
            ...
            self.assertEqual(row["story_estimate_hours"], 52.0)
            ...
            self.assertEqual(row["planned_total_hours"], 40.0)
            self.assertEqual(row["planned_to_date_hours"], 82.0)
            ...
            self.assertEqual(row["schedule_variance_hours"], -66.0)
            self.assertEqual(row["schedule_variance_pct"], -80.5)
            self.assertEqual(row["schedule_variance_hours_position"], "behind")
            self.assertEqual(row["estimation_accuracy_pct"], 250.0)
            self.assertEqual(row["estimation_accuracy_status"], "outside_ideal")
            self.assertEqual({item["issue_key"] for item in row["stories"]}, {"O2-1-S1", "O2-1-T1"})
            ...
            self.assertEqual(sv["planned_to_date_hours"], 82.0)
            self.assertEqual(sv["actual_to_date_hours"], 16.0)
            self.assertEqual(sv["schedule_variance_hours"], -66.0)
            ...
            self.assertEqual(alice_sv["planned_to_date_hours"], 16.0)
            self.assertEqual(alice_sv["actual_to_date_hours"], 10.0)
            self.assertEqual(alice_sv["schedule_variance_hours"], -6.0)
