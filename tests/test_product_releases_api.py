...
class ProductReleasesApiTests(unittest.TestCase):
    ...
    def test_release_readiness_design_contains_product_release_navigator(self):
        ...
        self.assertIn("function combineRecognizedReleases", script)
        self.assertIn("recognizedProjectKeys.has(release.project_key)", script)
        self.assertIn("releases = combineRecognizedReleases(results[0])", script)
        self.assertNotIn("releases = DEMO_RELEASES.concat", script)
