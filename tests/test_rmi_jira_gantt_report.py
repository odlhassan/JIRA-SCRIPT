...
class RmiJiraGanttReportTests(unittest.TestCase):
    ...
    def test_render_html_contains_report_controls_and_no_excel_source_text(self):
        ...
        self.assertIn("capacity-results-grid", html)
        ...
        self.assertIn("memberRecords = capacitySource.member_records || {}", html)
        self.assertIn("selectDefaultMembers()", html)
        self.assertIn("memberStatusLabel(name)", html)
        self.assertIn('Boolean(rec.resigned) ? "Resigned" : "Active"', html)
        self.assertIn("cb.indeterminate = selectedCount > 0 && selectedCount < members.length", html)
        self.assertIn("Select all (", html)
        self.assertIn('id="capacity-tk-approved-card" hidden', html)
        self.assertIn('id="capacity-idle-card" hidden', html)
        self.assertIn("monthToggleState()", html)
        self.assertIn('id="diagnostics-toggle-enabled"', html)
        self.assertIn('<span class="tk-month-toggle-text">Diagnostics</span>', html)
