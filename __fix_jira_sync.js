...
const filePath = String.raw`E:\JIRA SCRIPT\report_html\team_capacity_planner.html`;
...
// Pattern: from the Jira sync modal comment line up to (but not including) the Event listeners comment line
const pattern = /(  \/\/ \u2500\u2500 Jira sync modal \u2500+\n)[\s\S]*?(?=  \/\/ \u2500\u2500 Event listeners )/;
...
const newContent = content.replace(pattern, newSection);

fs.writeFileSync(filePath, newContent, 'utf8');

// Verify
const check = fs.readFileSync(filePath, 'utf8');
if (check.includes('_jiraSyncSelectedIds')) {
  console.log('SUCCESS: _jiraSyncSelectedIds is present in the file.');
} else {
  console.error('FAILURE: _jiraSyncSelectedIds NOT found after replacement.');
  process.exit(1);
}
