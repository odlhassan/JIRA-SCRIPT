...
- **The workflow pins the runner to Python 3.11 (`actions/setup-python@v5`) before vendoring.** App Service runs Python 3.11, and binary wheels are ABI-specific. Vendoring on the runner's default interpreter shipped `_imaging.cpython-312-*.so`, which 3.11 cannot load, so every project-image upload returned HTTP 500 with `ImportError: cannot import name '_imaging' from 'PIL'` even though the `PIL` directory was present. Pure-Python dependencies (Flask, Gunicorn, requests, openpyxl) were unaffected, which is why the failure only surfaced once Pillow was added. The verification step now globs for `_imaging.cpython-311-*.so` instead of only checking that `PIL/` exists, and `pip install` uses `--only-binary=:all:` so no dependency is silently built from source.
...
- If Azure restarts while Colossal Fetch is still acquiring Jira data, the next `/api/canonical-refresh/current` read or new refresh start marks the interrupted Fetch as `failed`. If the durable Fetch snapshot had already completed, recovery preserves the combined run as `fetch_ready` at the `fetch_done` step, clears any orphaned Compute row, and allows **Finish Refresh — Build Reports** to retry without fetching Jira again. Compute eligibility is validated from `canonical_fetch_runs`, so a legacy combined run row previously marked `failed` does not invalidate a completed Fetch snapshot. A second restart during the retry marks only the orphaned Compute as failed and leaves the same Fetch available for another Compute attempt.
...
- **Generators resolve their own writable output directory (`report_output_paths.py`).** Every generator anchors inputs and outputs to `Path(__file__).resolve().parent`, i.e. the app root — *not* to the process working directory. On a read-only package mount that produced `OSError: [Errno 30] Read-only file system: '/home/site/wwwroot/rlt_leave_report.html'` and failed the whole Compute phase. `report_output_paths.resolve_output_base(script_dir)` mirrors `report_server._canonical_bridge_artifact_base_dir()` exactly: it returns the script directory when writable (unchanged local behaviour), honours `JIRA_CANONICAL_ARTIFACT_DIR`, and otherwise returns `$HOME/data/canonical_artifacts`. Wired into `generate_rlt_leave_report.py` (xlsx/html/md outputs) and `generate_employee_performance_report.py` (`_resolve_runtime_paths(base_dir, output_base)` — xlsx inputs and HTML output move, the DB path does not, because its env var is absolute in production).
  - Setting cwd alone is **not** sufficient and was the first, incomplete attempt at this fix. `_run_script()` / `_run_script_interruptible()` still resolve the child cwd through `_resolve_script_cwd()`, which is correct for any script that does use relative paths, but the generators ignore cwd entirely.
- **Support Center DB falls back to a writable path.** `support_center_sync.resolve_db_path()` routes through `_writable_db_path()`, which keeps `support_center.db` where it is when the directory is writable and otherwise uses `$HOME/data/support_center.db`, **seeding it once from the deployed copy** so packaged support data is not lost. Prefer setting `SUPPORT_CENTER_DB_PATH=/home/data/support_center.db` explicitly, matching the other DB app settings.
- **Compute no longer fails on an unwritable publish target.** `sync_report_html()` at the end of `_run_canonical_compute()` is wrapped; a read-only `report_html` is recorded as `reports.sync_report_html_error` in the run stats and the Compute still completes, because derived tables, Epics Planner sync, and the canonical pointer are already durable in `/home/data`.
- **Promotion prefers the freshly generated copy.** `_resolve_output_html_path()` checks the writable artifact directory before the app root and returns whichever copy is newer, so a rebuild that landed in `$HOME/data/canonical_artifacts` is promoted instead of the stale HTML baked into the deploy package.
- `generate_rlt_leave_report.py` additionally stages its workbook in the system temp directory when `tempfile.mkstemp(dir=<target parent>)` is rejected, and warns (instead of raising) when the final `os.replace` into the target cannot be performed.
- **Known limitation — Compute completes but the live site keeps serving packaged HTML.** With `REPORT_HTML_DIR=report_html` the served directory is `/home/site/wwwroot/report_html`, which the package mount makes read-only, so promotion cannot land there no matter where generators write. DB-backed and API-driven reports do reflect a fresh Compute; static generated HTML does not. To publish generated HTML at runtime, point `REPORT_HTML_DIR` at a writable absolute path (for example `/home/data/report_html`) and seed it once from the deployed `report_html`, then restart the app.
...
- **"Create DB backup before Full Refresh" is not shown.** The `/settings/canonical-refresh` page used to expose a checkbox that copied the entire multi-GB capacity DB into `backups/canonical_refresh/` before a Full Refresh. This filled production disk, so the control was removed from the UI. `_start_canonical_refresh_async()` still forces `should_create_backup = False` regardless of the request payload, so no full-DB backup is written by this workflow. The backup helper (`_create_canonical_refresh_db_backup`) and API fields (`db_backup_requested`, `db_backup_created`, `db_backup_path`) remain for compatibility but are inert.
