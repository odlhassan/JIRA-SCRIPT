...
- **Generator subprocesses run in a writable working directory.** `_run_script()` / `_run_script_interruptible()` in `report_server.py` locate the script file under the app root but resolve the child process cwd through `_resolve_script_cwd()`, which returns the app root when it is writable and otherwise the same `$HOME/data/canonical_artifacts` directory used for compatibility artifacts. Generators write their `.xlsx`/`.html`/`.db` outputs relative to cwd, so before this guard the Compute phase died at `generate_rlt_leave_report.py` with `OSError: [Errno 30] Read-only file system: '/home/site/wwwroot/tmpXXXX.xlsx'` while the Fetch snapshot stayed valid. This also keeps generators reading the artifacts the bridge just wrote instead of stale copies baked into the deploy package.
