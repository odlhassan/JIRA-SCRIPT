...
- `generate_rlt_leave_report.py` — generator. Writes `rlt_leave_report.xlsx`, `rlt_leave_report.html`, and `RLT_LEAVE_REPORT.md` into `report_output_paths.resolve_output_base(<script dir>)` — the repo root locally, or the writable artifact directory when the app root is read-only. The workbook is written atomically (temp file, then `os.replace`); if the target directory rejects temp-file creation the workbook is staged in the system temp directory instead, and an unperformable final replace warns rather than raising.
- `report_output_paths.py` — shared writable-output-directory resolution used by the Colossal Refresh generator chain.
...
- `report_output_paths.resolve_output_base()` decides where this generator's outputs land. On a writable app root it is the repo root; on Azure's read-only `WEBSITE_RUN_FROM_PACKAGE` mount it is `$HOME/data/canonical_artifacts` (or `JIRA_CANONICAL_ARTIFACT_DIR`). `report_server._resolve_output_html_path()` then prefers whichever of the two copies of `rlt_leave_report.html` is newer when promoting into `report_html`. See `AZURE_APP_SERVICE.md`.
