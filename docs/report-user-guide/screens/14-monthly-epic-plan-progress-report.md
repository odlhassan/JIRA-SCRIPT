...
- `Brought Forward` is not a direct copy of the previous month's `Carrying to Next Month` total. It includes epics whose approved due date is before the selected month, inside the current `Overdue lookback` window, not on hold when `Include On Hold` is off, and not completed before the selected month started. A prior-month carry-out epic whose approved due date is in the selected month or later is counted in `Planned This Month` instead; an older overdue epic can be excluded by the lookback threshold.
...
| Epic Work Summary | Planned This Month / Brought Forward / Carrying to Next Month cards | Clickable cards | Calculated | Open the summary detail drawer. The Brought Forward drawer includes both included overdue epics and nearby carry-forward candidates that were not counted because their due date is in the selected month, they are outside the overdue lookback, they completed before the month, or they were excluded by filters. |
...
6. The service builds `carry_forward_audit`, which explains each Epic Work Summary decision: included brought-forward, in-month rather than carry-in, carrying to next month, completed before month, on-hold excluded, or outside the overdue lookback.
