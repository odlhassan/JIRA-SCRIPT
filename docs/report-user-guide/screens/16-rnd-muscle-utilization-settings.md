...
- **Resources** - Resources are stored with display name, initials, email, optional team membership, and explicit resource skills. Canonical assignee/worklog names are synced into `rnd_muscle_resources` on page-state load. The right resource catalog hides resources marked resigned in the source capacity database, groups the remaining active resources by assigned team, and keeps resources without a team under **No team** at the end of the list. Team membership is resolved from `resource.team_id` first and falls back to `team.resource_ids`, so saved team assignments still render under their team even when the resource row does not carry a populated `team_id`. The catalog search filters active resources while the user types across name, initials, email, and team name. Resource catalog rows use a light tone derived from the assigned team's color, with the original color preserved as an accent; resources without a team keep the neutral row treatment. Resource skill counts include only the skills selected directly for that resource.
...
- **Epic priority colors** - Epic rows and cards use a clean light-to-dark purple scale in the catalog, mapped-epic list, Cluster epic list, and backlog: priority 3 is light purple (`#EDE9FE`), priority 2 is medium purple (`#7C3AED`), and priority 1 is dark purple (`#4C1D95`). The cards use flat fills with no gradient, glow, or left accent stripe. Foreground, border, and metadata colors preserve readable contrast in both page themes.
