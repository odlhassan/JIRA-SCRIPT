...
- **Epics** - The epic catalog is sourced from `epics_management`. Epics whose latest matching `canonical_issues` Epic row has a `Completed` or `Resolved` status are excluded from the catalog, project counts, planner projections, backlog projection, and quick stats; punctuation variants such as `Resolved!` are normalized, while a later active status keeps a reopened epic visible. Priority comes from the `priority` column. Budgeted hours are derived from `epics_management_plan_values.epic_plan` as `man_days * 8`.
