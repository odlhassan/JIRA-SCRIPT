...
- **Resources** - Resources are stored with display name, initials, email, optional team membership, and explicit resource skills. Canonical assignee/worklog names are synced into `rnd_muscle_resources` on page-state load. Resource catalog rows use a light tone derived from the assigned team's color, with the original color preserved as an accent; resources without a team keep the neutral row treatment. Resource skill counts include only the skills selected directly for that resource. Resource rows show a compact **Resigned** chip when the source capacity database has a matching `performance_resource_resignations` record for that display name.
...
| Right panel | Scrollable resource list, team/skill badges, resigned badge, Edit skills action | Resources are loaded from `rnd_muscle_resources`; assigned-team rows use the team color, while no-team rows stay neutral. The skill badge counts only skills selected directly for that resource. A **Resigned** chip appears beside names found in `performance_resource_resignations`, with the resignation date available as hover text when present. Dragging a resource into the selected-epic resource area maps it. Hidden on the view route. |
