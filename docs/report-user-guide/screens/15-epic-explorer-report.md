...
| Table | SV Date / SV Hours | Signed KPI text | Blank when missing | SV Date keeps the epic schedule date basis. SV Hours is the Executive Summary budget variance: `total_actual_hours - Budget`, where Budget uses the positive TK budget or planned-total fallback shown in the same row. |
