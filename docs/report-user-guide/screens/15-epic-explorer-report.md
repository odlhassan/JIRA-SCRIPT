...
| Table | Jira Epics' Planned Dates | Two-line date display (`MMM-DD-YYYY`) | `S: -` / `D: -` when missing | Shows canonical epic `start_date` and `due_date` on separate, non-wrapping lines labeled `S:` and `D:`, for example `S: Jul-06-2026` and `D: Jul-24-2026`. |
...
| Table | Actual Complete Date | Date | Blank when unresolved or missing | For a currently resolved epic, the earlier of descendant subtask last worklog date and epic `resolved_stable_since_date`. The resolved date caps any later worklog date; when only one date exists, that available date is used. Unresolved and reopened epics remain blank. |
...
| Table | SV Date | Plain-language day variance with hover explanation | Blank when missing | Shows the absolute calendar-day difference as `late`, `early`, or `On time` instead of a signed number. Hovering shows unambiguous `DD MMM YYYY` dates, the calculation result, and whether the comparison uses Actual Complete Date for a resolved epic or today's reporting date for an open/reopened epic. The calculation remains Jira epic planned due date minus the applicable comparison date. |
...
| Table | Actual Complete Date | Date with hover tooltip (`MMM-DD-YY`) | Blank when missing | Row's `actual_complete_date`, displayed in `MMM-DD-YY`; hovering explains whether it came from the only available date or the earlier of last logged worklog and epic resolved-stable-since dates. |
...
- Actual Complete Date is populated only for epics whose current status is `Resolved`, `Resolved!`, `Done`, `Closed`, `Complete`, or `Completed`. For those epics, use the earlier of the last descendant worklog date and `resolved_stable_since_date` when both exist, otherwise use whichever exists. This makes the last logged date the completion date when work ended before resolution and caps a later worklog at the resolved date. Unresolved and reopened epics leave Actual Complete Date blank and use today as the SV reporting date, so their schedule variance continues to age even when no new worklogs are recorded.
- The resolved-date cutoff changes completion-date and schedule-date calculations only. It does not remove post-resolution worklog hours from Total Actual Hours, SV Hours, or employee-utilization totals; auditing or excluding those hours is a separate control.
- SV Date presents the existing signed calculation in plain language: a negative result is shown as `late`, a positive result as `early`, and zero as `On time`. Its hover text uses `DD MMM YYYY` dates and explicitly identifies the resolved-completion or current-reporting-date rule; SV Hours is unchanged.
...
- Reformatted the main-table Jira Epics' Planned Dates cell into compact `S:` and `D:` lines using three-letter month abbreviations and four-digit years so dates no longer wrap vertically in the narrow column.
