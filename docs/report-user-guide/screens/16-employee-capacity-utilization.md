...
- Booked Manhours is the original estimate of assigned subtasks overlapping the selected month. Logged Hours can use all employee worklogs or only worklogs against their assigned subtasks. RLT-board leave worklogs are excluded by default because leave hours are already reported in Leaves Taken; the user can explicitly include them when needed.
