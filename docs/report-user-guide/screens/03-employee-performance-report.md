...
- `Include support` and `Include resigned` now behave as default bulk selectors, not hard locks. When either toggle is enabled, matching members are auto-selected once, and users can still uncheck specific members afterward. Those manual member-level exclusions remain respected until the include toggle is changed again or filters are reset.
...
- The Employee Capacity & Utilization table uses the active capacity profile and the selected month. `Capacity (Hours)` already excludes the profile's official holiday dates; `Official Leaves` shows those excluded dates as a count and is not subtracted again. `Availability = Capacity - (planned leave + unplanned leave)`, while `Utilization = Logged Hours / Availability` and can exceed 100%.
- `Booked Manhours` is the sum of original estimates on non-leave subtasks assigned to the employee that overlap the selected month. The Logged hours control switches between every in-range worklog authored by the employee and only in-range worklogs on subtasks assigned to that same employee. Project filtering applies to the work data, but the table always considers all employees rather than inheriting the page Teams/member filter.
- The final `Grand Total` row sums Capacity, Leaves Taken, Availability, Booked Manhours, and Logged Hours across the displayed employees; its utilization is total logged hours divided by total availability. Official Leaves remains the selected profile's holiday-day count, rather than multiplying the same holidays by employee count.
- Canonical worklogs use the newest persisted canonical worklog run, even when the active canonical issue refresh is partial or issue-only. This matches Monthly Epic Plan Progress and prevents a newer refresh without worklogs from showing zero employee logged hours for a month that has canonical worklogs.
- `Display resigned` is off by default and hides employees marked resigned in `performance_resource_resignations`. `Indicate support` is on by default and adds a `Support` chip beside employees whose name is in `support_team_config.members_json`; it changes only the label, not which rows are displayed.
- Every employee-table value is a drilldown. The right drawer lists the raw assigned subtasks, worklogs, leave rows, or capacity-profile holidays behind the selected number; work item keys link to Jira.
...
| Date inputs accidentally contain `2026-04-01` to `2027-04-30` after a one-month April filter action | The report rewrites `To` to `2026-04-30`; Capacity Profile Expanded Settings shows April 2026 Business Days instead of a 13-month count. |
...
| Team | custom grouped multi-select | all teams selected | teams from `performance_teams` | Limits assignees to members of selected teams; supports individual member selection | Select only `SQA`; uncheck individual members to exclude them. |
...
| Include team | checkbox | checked | checked/unchecked | Selects or clears that team in the hidden `#teams` select; toggles all member checkboxes | Clear `Fullstack Team` to remove its members. |
| Include support | checkbox | unchecked | checked/unchecked | When unchecked (default), support-team members are auto-excluded. Checking it auto-selects support members as a starting point, but users can still uncheck specific support members via member checkboxes. Manual support-member exclusions are preserved while the toggle remains enabled. | Check it to bulk-include support roster, then uncheck specific support members you want excluded. |
| Include resigned | checkbox | unchecked | checked/unchecked | When unchecked (default), resigned resources whose resignation date is on or before the report `to` date are auto-excluded. Checking it auto-selects resigned members as a starting point, but users can still uncheck specific resigned members via member checkboxes. Manual resigned-member exclusions are preserved while the toggle remains enabled. | Check it to bulk-include resigned members, then uncheck specific resigned resources you still want excluded. |
| Member checkbox | checkbox per member | checked | checked/unchecked | Include or exclude an individual member from the report without deselecting the whole team | Uncheck `Aiza Hamid` to hide her row while keeping rest of DE Team visible. Trigger text shows `X of Y members` when individual filtering is active. |
...
| Capacity Profile | select | Auto | saved capacity profiles | Chooses capacity calendar for employee capacity. The expanded settings panel uses the active From/To window for Business Days and self-corrects the one-month/same-month-next-year date-entry mistake before calculating. | `Auto (Match selected date range)`. |
...
| Employee Capacity & Utilization Month | month input | active report month | calendar month | Writes the report From/To dates to that whole month and refreshes the table | Select `2026-03`. |
| Employee-table Logged hours | select | Any employee worklog | Any employee worklog, Only assigned subtasks | Selects all in-range employee worklogs or only worklogs against subtasks assigned to that employee | Choose `Only assigned subtasks`. |
| Display resigned | toggle | off | off/on | Includes resigned resources in the all-employees capacity table | Enable to review a resigned resource. |
| Indicate support | toggle | on | off/on | Shows a `Support` chip next to technical-support resources without changing the data | Turn off to hide chips. |
| Employee Capacity & Utilization values | clickable table cells | n/a | Capacity, official leave, personal leave, availability, booked, logged, utilization | Opens the right drawer with calculation inputs and raw source rows | Click `Logged Hours` to see linked work items and individual worklogs. |
...
6. Browser `renderAll()` normalizes the active From/To controls for the same-month-next-year one-month edge case, loads scoped subtasks for current filters, computes assignee metrics, applies team/project/date filters, and renders scorecards, leaderboard, charts, and drilldowns. Scoped subtask filtering uses date-overlap rules shared with Monthly Epic Plan Progress and Team Capacity Planner.
