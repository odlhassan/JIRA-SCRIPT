...
| Include support | checkbox | unchecked | checked/unchecked | When unchecked (default), all support-team members are auto-excluded from the report. Checking it adds them back. Respects the `supportTeamMemberSet` derived from `support_team_config.members_json`. | Check to include support-roster members in scores and KPIs. |
| Include resigned | checkbox | unchecked | checked/unchecked | When unchecked (default), resigned resources whose resignation date falls on or before the report "to" date are auto-excluded. Checking it includes them. Re-evaluated on every date change. | Check to include resigned employees in the leaderboard and KPIs. |
