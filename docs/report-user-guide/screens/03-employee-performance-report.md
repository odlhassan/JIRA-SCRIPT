...
- `Booked Manhours` is the sum of original estimates on non-leave subtasks assigned to the employee that overlap the selected month. The Logged hours control switches between every in-range worklog authored by the employee and only in-range worklogs on subtasks assigned to that same employee. Project and team/member filters apply to this table too.
